...
parser = argparse.ArgumentParser(
    ...
    epilog="Writes the images left to right, then top to bottom. U+XXX0 will be on the left edge, U+XXXF will be on the "
           "right edge. Writes glyphs with a transparent background and opaque text in the specified color. The color "
           "components of the background are still in the specified color to prevent interpolation artifacts."
)
parser.add_argument('-m', '--merge', action='store_true', help='Merge glyphs in the hex file with those already in the'
                                                               'atlas files and glyph width file, as opposed to '
                                                               'completely overwriting them')
parser.add_argument('-c', '--color', nargs=1, default='FFFFFF', help='The foreground color of the text in six-digit hex.')
...
parser.add_argument('glyph_width', help='The path for the glyph_width.bin output file')
