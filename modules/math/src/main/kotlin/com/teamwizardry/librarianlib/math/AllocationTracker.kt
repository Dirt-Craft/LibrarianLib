...
import com.teamwizardry.librarianlib.utils.GameTime
...
object AllocationTracker {
    var vec2dAllocations: Long = 0L
    var vec2dPooledAllocations: Long = 0L
    var vec2dAllocationStats: Object2IntMap<Vec2d>? = null
    var rect2dAllocations: Long = 0L
    var rect2dAllocationStats: Object2IntMap<Rect2d>? = null
    var vec3dAllocations: Long = 0L
    var vec3dPooledAllocations: Long = 0L
    var vec3dAllocationStats: Object2IntMap<Vec3d>? = null

    val vec2dSize: Int = 32
    val vec3dSize: Int = 40
    val rect2dSize: Int = 80
}

object AllocationDisplay {
    init { MinecraftForge.EVENT_BUS.register(this) }

    private val vec2dAllocations = AllocationWindow()
    private val vec2dPooledAllocations = AllocationWindow()
    private val rect2dAllocations = AllocationWindow()
    private val vec3dAllocations = AllocationWindow()
    private val vec3dPooledAllocations = AllocationWindow()
    private var statsStartTime: Int = 0
    private var statsEndTime: Int? = null
    private var statsOutputDir: String = "~"
    ...
    fun tick(event: TickEvent.ClientTickEvent) {
        ...
        if(statsEndTime?.let { GameTime.systemTimeTicks >= it } == true) {
            endStats()
            statsEndTime = null
        }
    }
    ...
    fun startStats(duration: Int?, outputDir: String = "~") {
        ...
        statsEndTime = duration?.let { GameTime.systemTimeTicks + it }
    }
    ...
    fun endStats() {
        try {
            ...
            val duration = (GameTime.systemTime - statsStartTime) / 20.0
    ...
    private const val B_PER_KB = 1024
    private const val B_PER_MB = 1024 * 1024
}

private class AllocationWindow {
    var windowSize = 5000
